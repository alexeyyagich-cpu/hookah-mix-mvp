...
export function QuickStatsRow({ sessions, avgRating, tips, kdsOrders, todayStr, tm }: QuickStatsRowProps) {
  ...
  const stats = [
    { label: String(tm.bossTodaySessions), value: String(todaySessions), color: 'var(--color-primary)' },
    { label: String(tm.bossAvgRating), value: avgRating > 0 ? avgRating.toFixed(1) : '—', color: 'var(--color-warning)' },
    { label: String(tm.bossTipsToday), value: todayTips > 0 ? `${Math.round(todayTips)}€` : '—', color: 'var(--color-success)' },
    { label: String(tm.bossKdsActive), value: String(activeKds), color: 'var(--color-info, var(--color-primary))' },
  ]
