...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchProfile = useCallback(async (userId: string) => {
    ...
    const { data, error } = await supabase
      .from('profiles')
      .select('id, business_name, owner_name, phone, address, logo_url, subscription_tier, subscription_expires_at, trial_expires_at, role, owner_profile_id, venue_slug, stripe_customer_id, stripe_subscription_id, onboarding_completed, onboarding_skipped, onboarding_step, business_type, active_modules, locale, created_at')
      .eq('id', userId)
      .single()

    if (error) {
      if (process.env.NODE_ENV !== 'production') console.error('Error fetching profile:', error.code, error.message)
      return null
    }
